 ...
 <div class="content">
   <div class="panel_s">
    <div class="panel-body">
      <div class="row">
        <div class="col-md-12">
         <h4><i class="fa fa-list-ul">&nbsp;&nbsp;</i><?php echo omni_htmldecode($title); ?></h4>
         <hr>
       </div>
     </div>
     <!-- Main content -->
     <div class="row">
      <div class="col-md-12">
        <?php
        echo form_open($this->uri->uri_string(), array('id' => 'order-form', 'class' => '_transaction_form_manual_order order-form')); ?>
        <div class="panel-body">
          <input type="hidden" name="id" value="<?php echo omni_htmldecode($id); ?>">
            ...
            <div class="row">
              <div class="col-md-12">
                <div class="form-group">
                  ...
                  <select name="estimate_id" id="estimate_id" class="selectpicker" data-live-search="true" data-width="100%" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>" <?php if(isset($order) && is_numeric($order->number_invoice)){ echo 'disabled';} ; ?> >
                    <option value=""></option>
                    <?php foreach($estimate_data as $estimate) { ?>
                      <option value="<?php echo omni_htmldecode($estimate['id']); ?>" <?php if(isset($order) && $order->estimate_id == $estimate['id']){ echo 'selected'; } ?>><?php echo format_estimate_number($estimate['id']); ?></option>
                    <?php } ?>
                  </select>
                </div>
              </div>
              ...
              <div class="col-md-12 form-group">
                ...
                <select name="customer" id="customer" class="selectpicker"data-live-search="true" data-width="100%" onchange="get_trade_discount()" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>" >
                  <option value=""></option>
                  <?php foreach ($customers as $s) { ?>
                    <option value="<?php echo omni_htmldecode($s['userid']); ?>" <?php if (isset($order) && $order->userid == $s['userid']) {echo 'selected';}?>><?php echo omni_htmldecode($s['company']); ?></option>
                  <?php }?>
                </select>
              </div>
                              ...
                              <div class="col-md-6">
                               <div class="form-group select-placeholder">
                                ...
                                <select name="discount_type" class="selectpicker" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                                 ...
                                 <option value="after_tax" <?php if((isset($order) && $order->discount_type_str == 'after_tax') || !isset($order)){ echo "selected"; } ?>  ><?php echo _l('discount_type_after_tax'); ?></option>
                               </select>
                             </div>
                           </div>
                         </div>
                    ...
                    <div class="items-select-wrapper">
                     <select name="item_select" class="selectpicker no-margin" data-width="100%" id="item_select" data-none-selected-text="<?php echo _l('add_item'); ?>" data-live-search="true">
                      ...
                      foreach ($items as $group_id => $_items) {
                        ?>
                        <optgroup data-group-id="<?php echo omni_htmldecode($group_id); ?>" label="<?php echo omni_htmldecode($_items[0]['group_name']); ?>">
                         <?php foreach ($_items as $item) {?>
                           <option value="<?php echo omni_htmldecode($item['id']); ?>" data-subtext="<?php echo strip_tags(mb_substr($item['long_description'] ?? '', 0, 200)) . '...'; ?>">(<?php echo app_format_number($item['rate']); ?>) <?php echo omni_htmldecode($item['description']); ?></option>
                         <?php }?>
                       </optgroup>
                     <?php }?>
                   </select>
                  ...
                  <div class="input-group input-group-select">
                   <select name="task_select" data-live-search="true" id="task_select" class="selectpicker no-margin _select_input_group" data-width="100%" data-none-selected-text="<?php echo _l('bill_tasks'); ?>">
                    ...
                    <?php foreach ($billable_tasks as $task_billable) {
                      ?>
                      <option value="<?php echo omni_htmldecode($task_billable['id']); ?>"<?php if ($task_billable['started_timers'] == true) {?>disabled class="text-danger important" data-subtext="<?php echo _l('invoice_task_billable_timers_found'); ?>" <?php } else {
                        ...
                        ?>
                        data-subtext="<?php echo omni_htmldecode($task_billable['rel_type']) == 'project' ? '' : omni_htmldecode($task_rel_value['name']); ?>" <?php }?>><?php echo omni_htmldecode($task_billable['name']); ?></option>
                      <?php }?>
                    </select>
           ...
           <table class="table invoice-items-table items table-main-invoice-edit has-calculations no-mtop">
            <thead>
             <tr>
              ...
              <th width="10%" align="right"><?php echo _l('discount'); ?>
              <span class="discount-column<?php echo ($discount_type != 1 ? ' hide' : '') ?>">(%)</span>
            </th>
         ...
         <td>
           <div class="form-group">
            <div class="quantity">
              ...
              <?php  echo render_input('unit_name', '', _l('unit'), 'text', ['placeholder' => _l('unit'), 'readonly' => true, 'disabled' => true], [], 'no-margin', 'input-transparent text-right wh_input_none'); ?>
            </div>
          </div>
        </td>
        <td>
      ...
      ?>
      <button type="button" onclick="add_item_to_table('undefined','undefined',<?php echo omni_htmldecode($new_item); ?>); return false;" class="btn pull-right btn-info"><i class="fa fa-check"></i></button>
    </td>
  </tr>

  <?php if ((isset($order) || isset($add_items) ) && (!is_numeric($order->estimate_id) || $order->estimate_id == 0) ) {
    $i               = 1;
    ...
    foreach ($add_items as $item) {
      ...
      $table_row .= render_input('unit_name', '', $unit_name, 'text', ['placeholder' => _l('unit'), 'readonly' => true, 'disabled' => true], [], 'no-margin', 'input-transparent text-right wh_input_none');
      $table_row .= '</div>';
      $table_row .= '</div></td>';
      ...
      $table_row .= '<td>';
      $table_row .= '<input name="' . $items_indicator . '[' . $i . '][discount]" class="form-control discount-item" value="'.($discount_type == 1 ? $item['percent_discount'] : $item['prices_discount']).'">';
      $table_row .= '</td>';

      $table_row .= '<td class="amount" align="right" data-id="'.$item['product_id'].'" data-gid="'.$group_id.'">';
      $table_row .= '<div class="price_w1 hide"><span class="old_price">'.$amount.'</span></div>';
      $table_row .= '<div class="price_w2 hide d-grid"><span class="new_price"></span><br><span class="line-throught old_price">'.$amount.'</span></div>';
      $table_row .= '</td>';
      ...
      $table_row .= '</tr>';
      echo omni_htmldecode($table_row);
      $i++;
    }
  }else{
    echo omni_htmldecode($order_manual_row_template);
  }
...
<div class="col-md-8 col-md-offset-4">
 <table class="table text-right">
  ...
  <?php 
  $add_discount = isset($order) ? $order->add_discount : 0;
  ?>

  <tr id="discount_area">
    <td>
      <div class="row">
        <div class="col-md-7 discount-title-fr">
          <span class="bold"><?php echo _l('discount'); ?>:</span>
          <input type="hidden" name="discount" value="<?php if(isset($order)){ echo $order->discount; } else { echo 0; } ?>">
        </div>
        <div class="col-md-3 add-discount-fr">
          <input name="add_discount" type="number" min="0" step="any" class="form-control" value="<?php echo omni_htmldecode($add_discount); ?>">
        </div>
        <div class="col-md-2 discount-type-fr">
          <select name="add_discount_type" class="selectpicker" data-width="100%" data-none-selected-text="None selected" tabindex="-98">
            <option value="1"<?php echo ($discount_type == 1 ? ' selected' : '') ?>>%</option>
            <option value="2"<?php echo ($discount_type == 2 ? ' selected' : '') ?>><?php echo _l('amount'); ?></option>
          </select>
        </div>
      </div>
    </td>
...
<tr class="<?php if(get_option('omni_sale_hide_shipping_fee') == 1){ echo "hide";} ?>">
  <td>
    <?php 
     ...
     ?>
    <div class="row">
      <div class="col-md-7 shipping_fee-title-fr">
        <span class="bold"><?php echo _l('omni_shipping_fee'); ?>:</span>
      </div>
      <?php if(get_option('omni_sale_hide_shipping_fee') == 0){ ?>
        <div class="col-md-3">
          <input name="shipping_value" type="number" min="0" step="any" class="form-control" value="<?php if(isset($order)){ echo $order->shipping_value; } else { echo get_option('omni_manual_shipping_fee'); } ?>">
        </div>
      <?php }else{ ?>
        <div class="col-md-3">
          <input name="shipping_value" type="number" min="0" step="any" class="form-control" value="0">
        </div>
      <?php } ?>

      <div class="col-md-2">
        <select name="shipping_form" class="selectpicker" data-width="100%" data-none-selected-text="None selected" tabindex="-98">
...
<tr class="hide">
  <td>
   ...
   <div class="col-md-5">
    <input type="number" data-toggle="tooltip" value="<?php if(isset($order)){ echo $order->shipping; } else { echo get_option('omni_manual_shipping_fee'); } ?>" class="form-control pull-left" name="shipping">
  </div>
</div>
</td>
...
<input type="hidden" name="amount_key" value="<?php echo _l('amount'); ?>">
...
<?php init_tail(); ?>
<?php require 'modules/omni_sales/assets/js/order_list/order_list_js.php';?>
</body>
</html>
