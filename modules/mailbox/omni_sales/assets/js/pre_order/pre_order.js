...
function calculate_total() {
    ...
    for(i = 0; i < qty.length; i++){
        var quantity = parseFloat(qty.eq(i).val());
        var price = parseFloat(rate.eq(i).val());
        // Old var discount_price = parseFloat(list_discount.eq(i).val());
        var amount = quantity * price;
        var amount_data = check_trade_discount_item(list_amount.eq(i), check_subtotal, amount, quantity);
        // Old amount = amount_data.amount;
        total_discount_item += amount_data.discount_item;
        // Old discount += quantity * discount_price;
        // Old list_amount.eq(i).text(ReplaceNumberWithCommas(amount));
        subtotal += amount;
        ...
        var it_dc_val = 0;
        var dc_price = amount;
        if(parseFloat(list_discount['minimum_order_value']) == 0){
            if(parseInt(list_discount['formal']) == 1){
              it_dc_val += amount*list_discount['discount']/100;
            }
            else{
             it_dc_val += list_discount['discount'];
            }
        }
        dc_price = dc_price - it_dc_val;

        let calculated_tax = (tax_rate * dc_price / 100);
