...
function customer_trade_discount(){
  "use strict";
  var tab = $('.tab-pane.active');
  var id = tab.find('select[name="client_id"]').val();
  tab.find('input[name="customer_id"]').val(id);
  if(id != ''){
    $.ajax({
    ...
    tab.find('input[name="discount_auto"]').val('0');
    var discount_voucher = tab.find('input[name="discount_voucher"]').val();
    if(discount_voucher == 0){
      tab.find('input[name="discount_type"]').val('');
      tab.find('input[name="discount_voucher_value"]').val();
    }
    total_cart(); 
  } 
}
