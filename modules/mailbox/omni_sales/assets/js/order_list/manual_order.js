...
(function($) {
    ...
    $("body").on('change', 'input[name="adjustment"],select.tax', function() {
        estimate_id = $('select[name="estimate_id"]').val();
    ...
    $("body").on('change', 'input[name="shipping_value"]', function() {
        estimate_id = $('select[name="estimate_id"]').val();
        if (!isNaN(estimate_id) && estimate_id != '') {
            wh_calculate_total();
        }else{
    ...
    $("body").on('change', 'select[name="shipping_form"]', function() {
        estimate_id = $('select[name="estimate_id"]').val();
        if (!isNaN(estimate_id) && estimate_id != '') {
            wh_calculate_total();
        }else{
...
function add_item_to_preview(id) {
    ...
    requestGetJSON('omni_sales/get_item_by_id/' + id).done(function(response) {
        ...
        $('.main .quantity input[name="unit_name"]').val(response.unitname);
    });
}
...
function add_item_to_table(data, itemid, merge_invoice, bill_expense) {
    ...
    var regex = /<br[^>]*>/gi;
        ...
        table_row += '<div class="form-group no-margin" app-field-wrapper="unit_name"><input type="text" id="unit_name" name="unit_name" class="form-control input-transparent text-right wh_input_none" placeholder="Unit" readonly="1" disabled="true" value="'+data.unit+'" aria-invalid="false"></div>';
        table_row += '</div>';
        table_row += '</div></td>';
...
function get_item_preview_values() {
    ...
    response.unit = $('.main .quantity input[name="unit_name"]').val();
    return response;
}
...
function calculate_total() {
    ...
    var shipping_value = $('input[name="shipping_value"]').val();
    var shipping_form = $('select[name="shipping_form"]').val();
    ...
    if(list_discount != undefined && list_discount['minimum_order_value'] != undefined && (cal_subtotal >= list_discount['minimum_order_value'])){
        if(list_discount['item'] == "" && list_discount['group_list'] == ""){
            discount_by_trade++;
            if (parseInt(list_discount['formal']) == 1) {
                if(discount_type == 'after_tax'){
                    total_discount_item = parseFloat(list_discount['discount']) * (cal_subtotal + cal_tax) / 100;
                }else{
        ...
        if(discount_value != '' && parseFloat(discount_value) > 0){
            ...
            if (parseInt(add_discount_type) == 1) {
                if(discount_type == 'after_tax'){
                    total_discount_item = parseFloat(discount_value) * (cal_subtotal + cal_tax) / 100;
                }else{
    ...
    $.each(rows, function(i, row_item) {
        ...
        if(discount_type == 'after_tax'){
            ...
            total_discount_item += (_amount + calculated_tax)*discount_value/100;
        }
        else{
            if( discount_type == 'before_tax'){

                if ((discount_percent !== '' && discount_percent != 0) &&  discount_total_type == 1) {
                    _amount = _amount - (_amount * discount_percent) / 100;
                }

                if(discount_value !== '' && discount_value != 0 && discount_total_type == 1){
                    _amount = _amount - (_amount * discount_value) / 100;
                }else if(discount_value !== '' && discount_value != 0 && discount_total_type == 2){
                    _amount = _amount - discount_value;
                }
    ...
    if(shipping_form == 'fixed'){
        ...
        shipping_fee = (parseFloat(total)*parseFloat(shipping_value)/100).toString();
    }

    total+= parseFloat(shipping_fee);
    $('input[name="shipping"]').val(parseFloat(shipping_fee));
    ...
    $('.shiping_fee').html(format_money(shipping_fee.toString()));
