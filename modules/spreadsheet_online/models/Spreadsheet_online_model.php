...
{	
	...
	{
		$data = $this->get_share_detail($root['id']);
		$html_staff = "";
    	$html_client = "";
    	if(count($data['staffs_share']) > 0){
			$new_data_array = [];
    		foreach ($data['staffs_share'] as $key => $value) {
				$per = $data['staffs_role'][$key] == 1 ? "V" : "E";
				$new_data_array[] = $value.'('.$per.')';
    		
    		}
			$html_staff .= implode(",",$new_data_array);
    	}

    	if(count($data['clients_share']) > 0){
			$new_data_array_client = [];
    		foreach ($data['clients_share'] as $key => $value) {
				$per = $data['clients_share'][$key] == 1 ? "V" : "E";
				$new_data_array_client[] = $value.'('.$per.')';
    		
    		}
			$html_client .= implode(",",$new_data_array_client);
    		
    	}
		$tree_tr = '';
		$class = "share-status"; 
		$html_change = '<i class="fa-regular fa-user menu-icon '.$class.'"></i>';
		...
		if($parent_id == ''){
			...
			$tree_tr .= '</td>';
			$tree_tr .= '<td>'.$html_staff.'</td>';
			$tree_tr .= '<td>'.$html_client.'</td>';
		}else{
