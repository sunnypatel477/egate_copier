...
function get_items_by_type_delivery($type, $ids,$item_order = "")
{
    $CI = &get_instance();
    $CI->db->select(db_prefix() . 'itemable.*, SUM('.db_prefix().'itemable.qty) as total_qty');
    $CI->db->from(db_prefix() . 'itemable');
    $CI->db->where(db_prefix() . 'itemable.rel_type', 'delivery_note');
    $CI->db->where_in(db_prefix() . 'itemable.rel_id', $ids);
    if($item_order != "") {
        $CI->db->where(db_prefix() . 'itemable.item_order', $item_order);
    }
    $CI->db->group_by(db_prefix() . 'itemable.item_order');
    return $CI->db->get()->result_array();
}

function get_items_by_type_item_order($type, $ids,$item_order = "")
{
    $CI = &get_instance();
    $CI->db->select();
    $CI->db->from(db_prefix() . 'itemable');
...
function get_items_by_type_item_order_wise($type, $ids, $item_order = "")
{
    $CI = &get_instance();
    $CI->db->select();
    $CI->db->from(db_prefix() . 'itemable');
    $CI->db->where_in('rel_id', $ids);
    $CI->db->where('rel_type', $type);
    $CI->db->where('item_order', $item_order);
    $CI->db->order_by('item_order', 'asc');
    ...
    // Loop through the result and reassign based on item_order
    foreach ($result as $item) {
        $ordered_items[$item['item_order']] = $item;
    }
...
function get_total_sum_of_delivery_notes($rel_id, $item_order)
{
    ...
    $CI->db->order_by(db_prefix() . 'itemable.item_order', 'asc');
    $query = $CI->db->get();
...
function count_delivery($type, $id)
{
    $CI = &get_instance();
    $CI->db->select('count(id) as countdelivery');
    $CI->db->from(db_prefix() . 'itemable');
    $CI->db->where('rel_id', $id);
    $CI->db->where('rel_type', $type);
    $CI->db->order_by('item_order', 'asc');
