                ...
                <div class="horizontal-tabs">
                    <ul class="nav nav-tabs nav-tabs-horizontal mbot15" role="tablist">
                        ...
                        <li role="presentation">
                            ...
                            $delivery_records = explode(',', $delivery_get_id);

                            foreach ($delivery_records as $key => $index) {
                                $total_count_delivery = 0;
                                if (!empty($index)) {
                                    $total_count_delivery = count($delivery_records);
                                }
                            }
                            ?>
                            <a href="#tab_purchase_order_delivery" aria-controls="tab_purchase_order_delivery" role="tab" data-toggle="tab">
                                <?php echo _l('purchase_order_delivery'); ?> <span style="color:red;">(<?php echo $total_count_delivery; ?>)</span>
                            </a>
                        </li>
                        <li role="presentation">
                ...
                <div class="col-md-9">
                    ...
                    <div class="pull-right _buttons">
                        ...
                        if (isset($purchase_order->delivery_noteid)) {
                            $remaining_qty_items_allow = 0;
                            foreach ($purchase_order->items as $key => $item) {
                                $delivery_get_ids = get_delivery_qty($purchase_order->id);
                                $delivery_history = explode(',',$delivery_get_id);

                                $total_qty = 0;
                                $purchase_qty = get_items_by_type_item_order_wise('purchase_order', $purchase_order->id,$key+1);

                               
                                $get_delivery_qty = get_items_by_type_delivery('delivery_note',  $delivery_history,$key+1);
                                
                             

                                $remaining_qty_items = $purchase_qty[$key+1]['qty'] - $get_delivery_qty[0]['total_qty'];
                                if($remaining_qty_items > 0){
                                    $remaining_qty_items_allow++;
                                }
                            }
                        }
                        ?>

                        <?php if (isset($purchase_order->delivery_noteid) && $remaining_qty_items_allow > 0) { ?>
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <?php
                                    if ($data['invoiceid']) { ?>
                                        <td><a href="<?php echo admin_url('invoices/list_invoices/' . $data['invoiceid']); ?>" data-placement="bottom" data-toggle="tooltip" title="<?php echo _l('delivery_note_invoiced_date', _dt($data['invoiced_date'])); ?>" class="btn btn-primary mleft10"><?php echo format_invoice_number($data['invoiceid']); ?></a></td>
                                    <?php } else { ?>
                                        <td></td>
                                    <?php } ?>
