...
class Articles extends AdminController
{
    ...
    public function article($id = '')
    {
        ...
        if ($this->input->post()) {
            if ($id == '') {
                ...
                if ($id) {
                    $uploadedFiles = handle_wiki_article_attachments_array($id);
                    if ($uploadedFiles && is_array($uploadedFiles)) {
                        foreach ($uploadedFiles as $file) {
                            $this->misc_model->add_attachment_to_database($id, WIKI_MODULE_NAME_ARTICAL, [$file]);
                        }
                    }
        ...
        if ($id == '') {
            ...
            if(isset($clone_id)){
                ...
                $data['attachments']    = $this->wikiarticles_model->get_wiki_article_attachments($id);
    ...
    public function show($id)
    {
        ...
        if (isset($data['articles'][0])) {
            ...
            $data['attachments']    = $this->wikiarticles_model->get_wiki_article_attachments($data['article']['id']);
    ...
    public function delete($id)
    {
        ...
        if ($response == true) {
            $this->wikiarticles_model->remove_wiki_article_all($id);
    ...
    {
        if ($this->input->is_ajax_request()) {
            echo json_encode($this->wikiarticles_model->remove_wiki_article_attachment($id));
        }
    }
    ...
    public function download_files($knowledge_base_id)
    {
        ...
        $files = $this->wikiarticles_model->get_wiki_article_attachments($knowledge_base_id, $taskWhere);
