...
class Client_lead_payments_model extends App_Model
{
    ...
    {
        $company = $this->db->select('userid, company')->get(db_prefix() . 'clients')->result_array();
        return $company;
    }
    ...
    public function _search_customer_leads($q, $limit = 0, $where = [])
    {
        $result = [
            'result'         => [],
            'type'           => 'leads',
        ...
        if (is_staff_member()) {
            ...
            if (!startsWith($q, '#')) {
                ...
                $tag = $this->db->escape_str(strafter($q, '#'));
                $this->db->where('id IN (
                    SELECT rel_id FROM ' . db_prefix() . 'taggables WHERE tag_id IN (
                        SELECT id FROM ' . db_prefix() . 'tags WHERE name = "' . $tag . '"
            ...
            if (!empty($where)) {
                $this->db->where($where);
                $this->db->where('status !=', CLOSED_LOST);
                $this->db->where('status !=', CLOSED_WON);
                $this->db->where('existing_client_id IS NOT NULL');
            }
    ...
    public function _search_client_leads($q, $limit = 0, $where = [])
    {
        $result = [
            'result'         => [],
            'type'           => 'leads',
        ...
        if (is_staff_member()) {
            ...
            if (!startsWith($q, '#')) {
                ...
                $this->db->where($where);
                $this->db->where('existing_client_id IS NULL');
