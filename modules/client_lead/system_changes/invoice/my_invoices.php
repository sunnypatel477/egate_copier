    ...
    ->outputUsing(function ($params) {
        ...
        $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
            ...
            '(SELECT SUM(amount) 
                ...
                WHERE invoiceid = tblinvoices.id AND paymentmode != 5) AS accountamount',
            '(SELECT SUM(amount) 
                FROM tblinvoicepaymentrecords 
                WHERE invoiceid = tblinvoices.id AND paymentmode = 5) AS paymentmodeamount',
        ]);
        ...
        foreach ($rResult as $aRow) {
            ...
            if (!empty($aRow['paymentmodeamount']) && !empty($aRow['accountamount'])) {
                $row[] = '<span style="color:red;">'.$aRow['accountamount'] .'</span>'.' + ' .'<span style="color:blue;">' . $aRow['paymentmodeamount'].'</span>';
            } elseif (!empty($aRow['paymentmodeamount'])) {
                $row[] = '<span style="color:blue;">'.$aRow['paymentmodeamount'].'</span>';
            } elseif (!empty($aRow['accountamount'])) {
                $row[] = '<span style="color:red;">'.$aRow['accountamount'] .'</span>';
            } else {
                $row[] = '';
            }
