...
return App_table::find('clients')
    ->outputUsing(function ($params) {
        ...
        $aColumns = [
            ...
            'COUNT(' . db_prefix() . 'leads.existing_client_id) AS lead',
        ...
        $join = [
            ...
            'LEFT JOIN ' . db_prefix() . 'leads ON ' . db_prefix() . 'leads.existing_client_id=' . db_prefix() . 'clients.userid  AND ' . db_prefix() . 'leads.status != ' . CLOSED_WON . ' AND ' . db_prefix() . 'leads.status != ' . CLOSED_LOST,
            'LEFT JOIN ' . db_prefix() . 'tasks ON ' . db_prefix() . 'tasks.rel_id=' . db_prefix() . 'clients.userid AND ' . db_prefix() . 'tasks.status = '.TASK_PENDING.' AND ' . db_prefix() . 'tasks.rel_type = "customer"',
        ];
        ...
        $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
            ...
            'COUNT(' . db_prefix() . 'tasks.id) AS pendingtask',
        ...
        foreach ($rResult as $aRow) {
            ...
            $row[] = ($aRow['lead'] ? '<span title="'._l('total_lead_title').'" class="lead-count">' . $aRow['lead'] . '</span>' : '') . 
            ($aRow['pendingtask'] ? ' <span title="'._l('pending_task_title').'" class="task-count">' . $aRow['pendingtask'] . '</span>' : '');
