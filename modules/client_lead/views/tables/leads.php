...
$this->ci->load->model(CLIENT_LEAD_MODULE . '/client_lead_payments_model');
$statuses = $this->ci->leads_model->get_status();
// $companys = $this->ci->client_lead_payments_model->get_company();
...
$rules = [
    ...
    App_table_filter::new('inquiry_about', 'TextRule')->label(_l('lead_add_edit_lead_inquiry_about')),
    // App_table_filter::new('company', 'TextRule')->label(_l('lead_add_edit_lead_company')),

    App_table_filter::new('company', 'MultiSelectRule')->label(_l('lead_add_edit_lead_company'))->options(function ($ci) {
        return collect($this->ci->client_lead_payments_model->get_company())->map(fn ($source) => [
...
return App_table::find('table-client-leads')
    ->outputUsing(function ($params) use ($statuses) {
        ...
        $aColumns = [
            ...
            db_prefix() . 'leads.existing_client_id as client_id',
            db_prefix() . 'leads.inquiry_about as inquiry_about',
            '1'
        ];
        ...
        if (isset($userid)) {
            array_push($where, 'AND existing_client_id = ' . $userid);
        }
        ...
        array_push($where, 'AND status != ' . CLOSED_LOST);
        array_push($where, 'AND status != ' . CLOSED_LOST);
        array_push($where, 'AND existing_client_id IS NOT NULL');
        ...
        foreach ($rResult as $aRow) {
            ...
            if ($aRow['client_id'] != 0  || $aRow['client_id'] != '') {
                $url = admin_url('clients/client/' . $aRow['client_id']);
                $row[] = ($aRow['company'] != '' ? '<a href="' . $url . '"><span title="'._l('total_lead_title').'" class="lead-count">' . lead_count($aRow['client_id']) . '</span> ' . $aRow['company'] . '</a>' : '');
            }

            

            $row[] = $aRow['inquiry_about'];
            $note_date = get_note_by_lead_id($aRow['id']);
            $count_reminder = get_note_by_lead_reminder($aRow['id']);


            if(isset($note_date->dateadded)) {
                $formatted_dateadded = e(time_ago($aRow['dateadded']));
                $tooltip_title = e(_dt($note_date->dateadded));
                $description_note = e($note_date->description);
                $totalnote = $count_reminder->totalnote;


                if($totalnote > 0) {
                    $totalnote =  '<span class="lead-count-note">' .$totalnote. ' </span> ';
                } else {
                    $totalnote = '';
                }

                $row[] = '<a href="#lead_notes" onclick="init_lead(' . $aRow['id'] . '); activateLeadNotesTab(); return false;" data-title="' . $tooltip_title . '" class="text-has-action is-date" title="' . _l($description_note) . '">'.$totalnote . $formatted_dateadded . '</a>';
                
            } else {
                $row[] = '';
            }

            $row[] = '<a>' . _l(isset($description_note) ? $description_note : '') . '</a>';
            
            $lastcontract_info = get_note_by_last_contarct_id($aRow['id']);

            if(isset($lastcontract_info->date)) {
                $formatted_date_last = e(time_ago($lastcontract_info->date));
                $tooltip_title = e(_dt($lastcontract_info->date));
                $description = e($lastcontract_info->description);

                $row[] = '<a href="#lead_activity" onclick="init_lead(' . $aRow['id'] . '); activatelastcontractTab(); return false;" data-title="' .  e(_dt($formatted_date_last)) . '" class="text-has-action is-date" title="' . _l($description) . '">' . $formatted_date_last . '</a>';
            } else {
                $row[] = '';
            }

            $row[] = '<a>' . _l(isset($description) ? $description: '') . '</a>';

            // $row[] = '<a href="#lead_activity" onclick="init_lead(' . $aRow['id'] . '); activatelastcontractTab(); return false;" data-title="' . e(_dt($aRow['lastcontact'])) . '" class="text-has-action is-date">' . e(time_ago($aRow['lastcontact'])) . '</a>';
...
<script>
    function activateLeadNotesTab() {
        $("li.active").removeClass("active");
        $("li a[href=\'#lead_notes\']").closest("li").addClass("active");
    }

    function activatelastcontractTab() {
        $("li.active").removeClass("active");
        $("li a[href=\'#lead_activity\']").closest("li").addClass("active");
    }
</script>