...
class Omni_invoices_model extends App_Model
{
    ...
    public function add($data, $expense = false)
    {
        ...
        if ($insert_id) {
			...
			$this->load->model('omni_sale/omni_sales_model');
            $this->omni_sales_model->after_invoice_added(['invoice_id' => $insert_id, 'order_id' => $order_id]);

            if(isset($hook['redeem'])){
                   hooks()->do_action('loy_after_invoice_added', $insert_id, $hook['redeem']);
            }
