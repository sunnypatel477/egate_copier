						...
						<div class="row">
							<div class="col-md-6">
								<h4 class="bold">
									<span id="invoice-number">
										<?php echo omni_htmldecode($order_return->order_return_number .' - '.$order_return->order_return_name); ?>
									</span>
								</h4>
								<address>
							...
							<div class="col-sm-6 text-right">
								<?php if($order_return->rel_type == 'manual' || $order_return->rel_type == 'sales_return_order'){ ?>
									...
									<address>
										<?php echo omni_htmldecode(get_company_name($order_return->company_id)) ?>
									</address>
								<?php }else{ ?>
									<span class="bold"><?php echo _l('wh_vendor'); ?>:</span>
									<address>
										<?php echo omni_htmldecode(get_vendor_company_name($order_return->company_id)) ?>
									</address>
								<?php } ?>
								<span class="bold"><?php echo _l('email'); ?>:</span>
									<?php echo omni_htmldecode($order_return->email) ?><br>
								<span class="bold"><?php echo _l('phonenumber'); ?>:</span>
									<?php echo omni_htmldecode($order_return->phonenumber) ?>
									...
									<table class="table items items-preview estimate-items-preview" data-type="estimate">
										<thead>
											<tr>
												...
												<th align="right" colspan="1"><?php echo omni_htmldecode($rate_label) ?></th>
										...
										<tbody class="ui-sortable">
											...
											foreach ($order_return_detail as $delivery => $order_return_value) {
												...
												if(omni_strlen($commodity_name) == 0){
												...
												<tr>
													<td ><?php echo omni_htmldecode($delivery) ?></td>
													<td ><?php echo omni_htmldecode($commodity_name) ?></td>
													<td class="text-right"><?php echo omni_htmldecode($quantity).$unit_name ?></td>
													...
													<td class="text-right"><?php echo omni_htmldecode($order_return_value['reason_return']) ?></td>
												</tr>
											<?php  } ?>
										</tbody>
									</table>
										...
										<table class="table text-right">
											<tbody>
												...
												<?php if(isset($order_return) && $tax_data['html_currency'] != ''){
													echo omni_htmldecode($tax_data['html_currency']);
												} ?>
							...
							<div class="col-md-12">
													...
													if($value['action'] == 'sign'){
														...
														<div class="col-md-3 text-center">
															<p class="text-uppercase text-muted no-mtop bold">
																...
																echo omni_htmldecode($staff_name); 
															?></p>
															...
															<p class="text-muted no-mtop bold">  
																<?php echo omni_htmldecode($value['note']) ?>
															</p>   
														</div>
													<?php }else{ ?>
														<div class="col-md-3 text-center">
									...
									if($order_return->approval != 1 && ($check_approve_status == false ))
										...
										{ ?>
											<?php if($check_appr && $check_appr != false){ ?>
												<a data-toggle="tooltip" class="btn btn-success lead-top-btn lead-view send_request_approve_class" data-placement="top" href="#" onclick="send_request_approve(<?php echo omni_htmldecode($order_return->id); ?>); return false;"><?php echo _l('send_request_approve'); ?></a>
											<?php } ?>
													...
													<ul class="dropdown-menu dropdown-menu-right menu-width-height" >
														...
														<li>
															<div class="row text-right col-md-12">
																<a href="#" onclick="approve_request(<?php echo omni_htmldecode($order_return->id); ?>); return false;" class="btn btn-success button-margin approve_request_class" ><?php echo _l('approve'); ?></a>
																<a href="#" data-loading-text="<?php echo _l('wait_text'); ?>" onclick="deny_request(<?php echo omni_htmldecode($order_return->id); ?>); return false;" class="btn btn-warning"><?php echo _l('deny'); ?></a></div>
															</li>
														</ul>
													</div>
												<?php }
								...
								<?php if($order_return->admin_note != ''){ ?>
									<div class="col-md-12 row mtop15">
										...
										<p><?php echo omni_htmldecode($order_return->admin_note); ?></p>
									</div>
								<?php } ?>
								<?php if($order_return->return_policies_information != ''){ ?>
									<div class="col-md-12 row mtop15">
										<p class="bold text-muted"><?php echo _l('wh_return_policies_information'); ?></p>
										<p><?php echo omni_htmldecode($order_return->return_policies_information); ?></p>
									</div>
								<?php } ?>
										...
										<div class="feed-item">
											<div class="date">
												...
												<?php if($log['staffid'] == get_staff_user_id() || is_admin() || has_permission('warehouse','','delete()')){ ?>
													<a href="#" class="pull-right text-danger" onclick="delete_wh_activitylog(this,<?php echo omni_htmldecode($log['id']); ?>);return false;"><i class="fa fa fa-times"></i></a>
												<?php } ?>
											</div>
											<div class="text">
												...
												if(!empty($log['additional_data'])){
													...
													echo omni_htmldecode($log['full_name']) . ' - ';
						...
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<div class="modal-footer">
									...
									<button onclick="sign_request(<?php echo omni_htmldecode($order_return->id); ?>);" autocomplete="off" class="btn btn-success sign_request_class"><?php echo _l('e_signature_sign'); ?></button>
								</div>
							</div>
						</div>
					</div>
