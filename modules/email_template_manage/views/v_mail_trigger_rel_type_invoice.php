...
require_once __DIR__ ."/v_mail_trigger_rel_type_option_inc.php";
...
<div class="col-md-12">
    ...
    <div class="form-group">
        ...
        <label class="control-label"><?php echo _l('email_template_manage_invoice_status_select')?></label>
        ...
        <select class="selectpicker" required="required" multiple="multiple" id="status" name="status[]"
                ...
                data-none-selected-text="<?php echo _l('email_template_manage_invoice_status_select')?>" >

            <?php

            foreach ( $status as $statu )
            {
                $status_text = format_invoice_status( $statu , '' , false );

                $selected = '';
                if( in_array( $statu , $record_status ) )
                    $selected = 'selected';

                echo "<option $selected value='$statu'>$status_text</option>";
            }
...
<div class="col-md-6">
    <select class="selectpicker" required="required" id="record_date" name="record_date" data-width="100%" data-show-subtext="true"
        ...
        <option></option>
        <option <?php echo $record_date == 'date' ? 'selected' : '' ?>   value="date"><?php echo str_replace( ':' , '' , _l('invoice_data_date') )?></option>
        <option <?php echo $record_date == 'duedate' ? 'selected' : '' ?> value="duedate"><?php echo str_replace( ':' , '' , _l('invoice_data_duedate') )?></option>
    </select>
</div>
...
<div class="col-md-6">
    ...
    <select class="selectpicker" id="record_day" name="record_day" data-width="100%" data-show-subtext="true" >
        ...
        <option <?php echo ( $record_day == '0' ? 'selected' : '' ) ?> value="0"><?php echo _l('email_template_manage_day_equal')?></option>
