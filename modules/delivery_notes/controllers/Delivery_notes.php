    ...
    public function delivery_note($id = '')
    {
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                foreach($delivery_note_data['items'] as $key => $item) {
                    $last_qty = get_items_by_type_item_order_wise('delivery_note', $id, $key);
                    $current_delivery_qty = $delivery_note_data['items'];
                    $remaining_qty_items =  $last_qty[$key]['qty'] - $current_delivery_qty[$key]['qty'];
                        if ($remaining_qty_items < 0) {
                            set_alert('danger', _l('qty_is_not_allowed_more_then_purchase_qty'));
