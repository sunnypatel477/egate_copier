...
class Delivery_notes_model extends App_Model
{
    ...
    public function convert_from_purchase_order($purchase_orderid, $status)
    {
        ...
        foreach ($_purchase_order->items as $key => $item) {
            ...
            $delivery_get_id = get_delivery_qty($_purchase_order->id);
            ...
            $purchase_qty = get_items_by_type_multiple('purchase_order', $purchase_orderid);
            ...
            if (isset($_purchase_order->delivery_noteid) && $_purchase_order->delivery_noteid != '') {
                $new_delivery_note_data['newitems'][$key]['qty'] =  $remaining_qty_items[$key];
            } else {
                $new_delivery_note_data['newitems'][$key]['qty'] = $_purchase_order->items[$key]['qty'];
            }
        ...
        foreach($new_delivery_note_data['newitems'] as $ne_key=>$ne){
            if($ne['qty'] <= 0){
                unset($new_delivery_note_data['newitems'][$ne_key]);
            }
        }
        $id = $this->add($new_delivery_note_data);
