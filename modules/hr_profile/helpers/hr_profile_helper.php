...
function get_discipline_name($id)
{
	if (is_numeric($id)) {
		$CI = &get_instance();
		$CI->db->where('id', $id);
		$CI->db->select('name');
		$result =  $CI->db->get(db_prefix() . 'hr_disciplinary_template')->row();
		return isset($result->name) ? $result->name : '';
	}
}

function get_discipline_subject($id)
{
	$CI = &get_instance();
	$CI->db->where('id', $id);
	$CI->db->select('discipline_reason_name');
	$result =  $CI->db->get(db_prefix() . 'hr_disciplinary_history_templete')->row();
	return isset($result->discipline_reason_name) ? $result->discipline_reason_name : '';
}

function get_penalty_data($id = false)
{
	$CI = &get_instance();
	if (is_numeric($id)) {
		$CI->db->where('id', $id);
		return $CI->db->get(db_prefix() . 'hr_penalty_template')->row();
	}
	if ($id == '') {
		return  $CI->db->get(db_prefix() . 'hr_penalty_template')->result_array();
	}
}

function discipline_note_pdf($discipline)
{
    return app_pdf('discipline_custom', module_libs_path(HR_PROFILE_MODULE_NAME) . 'pdf/discipline_report', $discipline);
}