...
if (!$CI->db->table_exists(db_prefix() . 'hr_disciplinary_template')) {
  $CI->db->query('CREATE TABLE `' . db_prefix() . "hr_disciplinary_template` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `name` TEXT NULL,
  `job_position` LONGTEXT NULL,
  `content` LONGTEXT NULL,
...
if (!$CI->db->table_exists(db_prefix() . 'hr_penalty_template')) {
  $CI->db->query('CREATE TABLE `' . db_prefix() . "hr_penalty_template` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `name` TEXT NULL,
  `job_position` LONGTEXT NULL,
  `content` LONGTEXT NULL,
...
if (!$CI->db->field_exists('point', db_prefix() . 'hr_penalty_template')) {
  $CI->db->query('ALTER TABLE `' . db_prefix() . "hr_penalty_template`
    ADD COLUMN `point` int(255) NULL
  ;");
}

if (!$CI->db->table_exists(db_prefix() . 'hr_disciplinary_history_templete')) {
  $CI->db->query('CREATE TABLE `' . db_prefix() . "hr_disciplinary_history_templete` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `discipline_reason_name` TEXT NULL,
  `discipline_category` int(11) NOT NULL,
  `discipline_reason_remarks` LONGTEXT NULL,
  `company_rules` LONGTEXT NULL,
...
if (!$CI->db->table_exists(db_prefix() . 'disciplinary_history')) {
  $CI->db->query('CREATE TABLE `' . db_prefix() . "disciplinary_history` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
...
if (!$CI->db->field_exists('content_template', db_prefix() . 'disciplinary_history')) {
  $CI->db->query('ALTER TABLE `' . db_prefix() . "disciplinary_history`
    ADD COLUMN `content_template` VARCHAR(100) NULL
      ;");
}

if (!$CI->db->field_exists('penalty_point', db_prefix() . 'disciplinary_history')) {
  $CI->db->query('ALTER TABLE `' . db_prefix() . "disciplinary_history`
    ADD COLUMN `penalty_point` LONGTEXT NULL
      ;");
}
...
if (!$CI->db->table_exists(db_prefix() . 'issued_assets_bnefits')) {
  $CI->db->query('CREATE TABLE `' . db_prefix() . "issued_assets_bnefits` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `staff_id` int(100) NOT NULL,
  `issued_assets_bnefits_name` VARCHAR(32) NOT NULL,
...
if (!$CI->db->field_exists('issued_date', db_prefix() . 'issued_assets_bnefits')) {
  $CI->db->query('ALTER TABLE `' . db_prefix() . "issued_assets_bnefits`
    ADD COLUMN `issued_date` date NOT NULL
      ;");
}

if (!$CI->db->table_exists(db_prefix() . 'job_required_document')) {
  $CI->db->query('CREATE TABLE `' . db_prefix() . "job_required_document` (
        `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
       `document_type` varchar(200) NOT NULL,
       `expiration_date` int(11) NULL,
        PRIMARY KEY (`id`)
      ) ENGINE=InnoDB DEFAULT CHARSET=" . $CI->db->char_set . ';');
}

if (!$CI->db->table_exists(db_prefix() . 'rec_required_document')) {
  $CI->db->query('CREATE TABLE `' . db_prefix() . "rec_required_document` (
      `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
      `staff_id` int(11) NOT NULL,
      `document_type` varchar(200) NOT NULL,
      `licence_no` varchar(200)  NULL,
      `acquisition_date` date NULL,
      `expiry_date` date NULL,
      `issue_authority` varchar(200)  NULL,
...
if (!$CI->db->field_exists('contract_templete_select', db_prefix() . 'hr_staff_contract')) {
  $CI->db->query('ALTER TABLE `' . db_prefix() . "hr_staff_contract`
    ADD COLUMN `contract_templete_select` int(11)  NULL
      ;");
}
