...
class Hr_profile extends AdminController
{
	...
	public function setting()
	{
		...
		$data['tab'][] = 'disciplinary_templates';
		$data['tab'][] = 'disciplinary_action_templates';
		$data['tab'][] = 'required_document_201';
		...
		} elseif ($data['group'] == 'disciplinary_templates') {
			$data['disciplinary_templates'] = $this->hr_profile_model->get_disciplinary_template();
			$data['penalty_templates'] = $this->hr_profile_model->get_penalty_template();
		} elseif ($data['group'] == 'disciplinary_action_templates') {
			$data['disciplinary_action_templates'] = $this->hr_profile_model->get_disciplinary_action_history();
			$data['disciplinary'] = $this->hr_profile_model->get_disciplinary_template();
		} elseif ($data['group'] == 'required_document_201') {
			$data['required_document_list']  = $this->hr_profile_model->get_required_document_list();
		}
	...
	public function member_contract_view($id = '', $group = '', $contract_id = '', $staff_id = '')
	{
		$data['staffid'] = $id;
		$data['group'] = $group;
		...
		$data['tab'][] = 'disciplinary_action_history';
		$data['tab'][] = 'issued_assets_benefits';
		$data['tab'][] = 'attach';
		...
		if ($id == '') {
			...
			if ($data['group'] == 'disciplinary_action_history') {
				$data['disciplinary'] = $this->hr_profile_model->get_disciplinary_template();
				$data['penalty'] = $this->hr_profile_model->get_penalty_template();
				$data['disciplinary_subject'] = $this->hr_profile_model->get_disciplinary_subject();
				$data['action_taken'] = $this->hr_profile_model->get_action_taken();
				$data['disciplinary_history'] = $this->hr_profile_model->get_hr_disciplinary_history($id);
			}
		...
		if ($this->input->post()) {
			...
			if ($contract_id == '' || $contract_id == 0) {
				...
				$contract_id = $this->hr_profile_model->add_contract($data);
				...
				if ($contract_id) {
					...
					$uploadedFiles = hr_profile_handle_contract_attachments_array($contract_id, 'file');

					if ($uploadedFiles && is_array($uploadedFiles)) {
						foreach ($uploadedFiles as $file) {

							$insert_file_id = $this->hr_profile_model->add_attachment_to_database($contract_id, 'hr_contract', [$file]);
						}
					}
				}
				...
				if (!$staff_id) {
					...
					redirect(admin_url('hr_profile/contracts/' . $contract_id));
				} else {
					set_alert('success', _l('added_successfully', _l('contract')));
					redirect(admin_url('hr_profile/member/' . $staff_id . '/contract'));
				}
			} else {
				if ($contract_id > 0) {
					...
					$response = $this->hr_profile_model->update_contract($data, $contract_id);
					//upload file
					if ($contract_id) {
						$success = true;
						$_id = $contract_id;
		...
		if ($contract_id == '' || $contract_id == 0) {
			...
			if ($contract_id > 0) {
				...
				$contract = $this->hr_profile_model->get_contract($contract_id);
				...
				$data['contract_attachment'] = $this->hr_profile_model->get_hrm_attachments_file($contract_id, 'hr_contract');
				if (!$contract) {
		...
		$this->load->view('hr_record/member_contract', $data);
	}
	...
	public function member($id = '', $group = '')
	{
		...
		if ($id == '') {
			...
			if ($data['group'] == 'required_document_201') {
				$data['required_document_201'] = $this->hr_profile_model->get_required_document_201($id);
				$data['staff_id'] = $id;
			}
			if ($data['group'] == 'attach') {
				$data['hr_profile_staff'] = $this->hr_profile_model->get_hr_profile_attachments($id);
	 ...
	 * @return [type]
	 */
	public function contract($id = '', $staff_id = '')
	{
		if (!has_permission('hrm_contract', '', 'view') && !has_permission('hrm_contract', '', 'view_own') && !is_admin()) {
			access_denied('staff_contract');
		}
	 ...
	 * @return [type]
	 */
	public function table_disciplinary_history()
	{
		$this->app->get_table_data(module_views_path('hr_profile', 'dependent_person/table_disciplinary_history'));
	}
	...
	public function reports()
	{
		...
		$data['contract_code'] = $this->hr_profile_model->get_contract();
	...
	public function list_of_discipline_report()
	{

		if ($this->input->is_ajax_request()) {
			$months_report = $this->input->post('months_filter');
			$staff_filter = $this->input->post('staff_filter_new');
			...
			if ($months_report) {
				if ($from_date && $to_date) {
					array_push($where, "AND date BETWEEN '$from_date' AND '$to_date'");
				}
			}

			if ($staff_filter) {
				array_push($where, 'AND staff_id IN (' . implode(', ', $staff_filter) . ')');
			}
			...
			$result = data_tables_init($aColumns, 'id', $sTable, [], $where, ['action_taken'], 'group by ' . db_prefix() . 'disciplinary_history.id');
	...
	public function list_of_employee_contract_report()
	{
		if ($this->input->is_ajax_request()) {

			$staff_filter = $this->input->post('staff_filter_contract');
			...
			if ($months_report) {
				if ($from_date && $to_date) {
					array_push($where, "AND start_valid BETWEEN '$from_date' AND '$to_date'");
				}
			}
			...
			$select = [
				'staff',
				'contract_code',
				db_prefix() . 'hr_staff_contract_type.name_contracttype as name_contracttype',
				'start_valid',
				'end_valid',
				'contract_status',
				'sign_day'
			...
			if ($staff_filter) {
				array_push($where, 'AND staff IN (' . implode(', ', $staff_filter) . ')');
			}

			if ($contract_code) {
				array_push($where, 'AND id_contract IN (' . implode(', ', $contract_code) . ')');
			}
			...
			$result = data_tables_init($aColumns, 'id_contract', $sTable, $join, $where, ['id_contract'], 'group by ' . db_prefix() . 'hr_staff_contract.id_contract');
			...
			foreach ($rResult as $aRow) {
				...
				if (isset($member_view) && $member_view == '1') {
					$subjectOutput .= '<a href="#" onclick="member_view_contract(' . $aRow['id_contract'] . ');return false;">' . _l('hr_view') . ' </a>';
					if ($aRow['signature'] != '') {
						$subjectOutput .= ' | <a href="' . admin_url('hr_profile/contract_sign/' . $aRow['id_contract']) . '" >' . _l('hr_view_detail') . ' </a>';
					}
				} else {
					if (has_permission('hrm_contract', '', 'view') || has_permission('hrm_contract', '', 'view_own') || is_admin()) {
						$subjectOutput .= '<a href="' . admin_url('hr_profile/contracts/' . $aRow['id_contract']) . '" onclick="init_hrm_contract(' . $aRow['id_contract'] . ');return false;">' . _l('hr_view') . ' </a>';
					}
					// if (has_permission('hrm_contract', '', 'edit') || is_admin()) {
	...
	/**
	 * delete disciplinary type
	 * @param  integer $id
	 */
	public function delete_disciplinary_type($id)
	{
		if (!$id) {
			redirect(admin_url('hr_profile/setting?group=disciplinary_templates'));
		}
		$response = $this->hr_profile_model->delete_disciplinary_type($id);
		if (is_array($response) && isset($response['referenced'])) {
			set_alert('warning', _l('hr_is_referenced', _l('disciplinary_templates')));
		} elseif ($response == true) {
			set_alert('success', _l('deleted', _l('disciplinary_templates')));
		} else {
			set_alert('warning', _l('problem_deleting', _l('disciplinary_templates')));
		}
		redirect(admin_url('hr_profile/setting?group=disciplinary_templates'));
	}
	 ...
	 * @param  integer $id
	 */
	public function delete_disciplinary_action_type($id)
	{
		if (!$id) {
			redirect(admin_url('hr_profile/setting?group=disciplinary_action_templates'));
		}
		$response = $this->hr_profile_model->delete_disciplinary_action_type($id);
		if (is_array($response) && isset($response['referenced'])) {
			set_alert('warning', _l('hr_is_referenced', _l('disciplinary_action_templates')));
		} elseif ($response == true) {
			set_alert('success', _l('deleted', _l('disciplinary_action_templates')));
		} else {
			set_alert('warning', _l('problem_deleting', _l('disciplinary_action_templates')));
		}
		redirect(admin_url('hr_profile/setting?group=disciplinary_action_templates'));
	}
	 ...
	 * @return [type]
	 */
	public function get_disciplinary_template($id = '')
	{
		$disciplinary = $this->hr_profile_model->get_disciplinary_template($id);
	 ...
	 * @return [type]
	 */
	public function get_disciplinary_action($id = '')
	{
		$disciplinary = $this->hr_profile_model->get_disciplinary_action($id);
		echo json_encode([
			'disciplinary_action_type' => $disciplinary,
		]);
		die;
	}
	 ...
	 * @return [type]
	 */
	public function get_disciplinary_templetes($id = '')
	{
		$filetmpDir = base_url() . 'modules/' .  HR_PROFILE_MODULE_NAME . '/uploads/disciplinary_file';
		$disciplinary_templete = $this->hr_profile_model->get_disciplinary_templetes($id);
		$file_path = $filetmpDir . '/' . $disciplinary_templete->file;
		// if (file_exists($file_path)) {
		// } else {
		// }
		echo json_encode([
			'disciplinary_templete' => $disciplinary_templete,
			'file_path' => $file_path
		]);
		die;
	}

	public function delete_disciplinary_templetes($id = '')
	{
		if ($id != '' && $id > 0) {
			$filetmpDir_o = base_url() . 'modules/' .  HR_PROFILE_MODULE_NAME . '/uploads/disciplinary_file';
			$filetmpDir = module_dir_path('hr_profile/uploads/disciplinary_file');
			$disciplinary_templete = $this->hr_profile_model->get_disciplinary_templetes($id);
			...
			$delete_disciplinary_templetes = $this->hr_profile_model->delete_disciplinary_templetes($id);
			if ($delete_disciplinary_templetes) {
				set_alert('success', _l('delete_disciplinary_templete'));
			}
		}

		redirect(admin_url('hr_profile/member/' . $disciplinary_templete->staff_id . '/disciplinary_action_history'));
		die;
	}
	 ...
	 * @param  integer $id
	 */
	public function disciplinary_type($id = '')
	{
		...
		if ($this->input->post()) {
			...
			if (!$this->input->post('id')) {
				...
				$id = $this->hr_profile_model->add_disciplinary_type($data);
				if ($id) {
					$success = true;
					$message = _l('added_successfully', _l('disciplinary_type'));
					set_alert('success', $message);
				}
				redirect(admin_url('hr_profile/setting?group=disciplinary_templates'));
			} else {
				$id = $data['id'];
				unset($data['id']);
				$success = $this->hr_profile_model->update_disciplinary_type($data, $id);
				if ($success) {
					$message = _l('updated_successfully', _l('disciplinary_type'));
					set_alert('success', $message);
				} else {
					$message = _l('hr_updated_failed', _l('disciplinary_type'));
					set_alert('warning', $message);
				}
	 ...
	 * @param  integer $id
	 */
	public function disciplinary_action_history($id = '')
	{
		...
		if ($this->input->post()) {
			...
			if (!$this->input->post('id')) {
				...
				$id = $this->hr_profile_model->add_disciplinary_action_history($data);
				if ($id) {
					$success = true;
					$message = _l('added_successfully', _l('disciplinary_action_templates'));
					set_alert('success', $message);
				}
				redirect(admin_url('hr_profile/setting?group=disciplinary_action_templates'));
			} else {
				$id = $data['id'];
				unset($data['id']);
				$success = $this->hr_profile_model->update_disciplinary_action_history($data, $id);
				if ($success) {
					$message = _l('updated_successfully', _l('disciplinary_action_templates'));
					set_alert('success', $message);
				} else {
					$message = _l('hr_updated_failed', _l('disciplinary_action_templates'));
					set_alert('warning', $message);
				}
	 ...
	 * @param  integer $id
	 */
	public function penalty_type($id = '')
	{
		...
		if ($this->input->post()) {
			...
			if (!$this->input->post('id')) {
				...
				$id = $this->hr_profile_model->add_penalty_type($data);
				if ($id) {
					$success = true;
					$message = _l('added_successfully', _l('penalty_type'));
					set_alert('success', $message);
				}
				redirect(admin_url('hr_profile/setting?group=disciplinary_templates'));
			} else {
				$id = $data['id'];
				unset($data['id']);
				$success = $this->hr_profile_model->update_penalty_type($data, $id);
				if ($success) {
					$message = _l('updated_successfully', _l('penalty_type'));
					set_alert('success', $message);
				} else {
					$message = _l('hr_updated_failed', _l('penalty_type'));
					set_alert('warning', $message);
				}
	 ...
	 * @param  integer $id
	 */
	public function delete_penalty_type($id)
	{
		if (!$id) {
			redirect(admin_url('hr_profile/setting?group=disciplinary_templates'));
		}
		$response = $this->hr_profile_model->delete_penalty_type($id);
		if (is_array($response) && isset($response['referenced'])) {
			set_alert('warning', _l('hr_is_referenced', _l('disciplinary_templates')));
		} elseif ($response == true) {
			set_alert('success', _l('deleted', _l('disciplinary_templates')));
	 ...
	 * @return [type]
	 */
	public function get_penalty_template($id = '')
	{
		$penalty = $this->hr_profile_model->get_penalty_template($id);
	 ...
	 * @param  integer $id
	 */
	public function disciplinary_history($id = '')
	{
		...
		if ($this->input->post()) {
			...
			if (!$this->input->post('id')) {
				...
				if ($_FILES['file']['name'] != "") {
					...
					$newFilePath =  $filetmpDir . $filename;
					...
					if (move_uploaded_file($tmpFilePath, $newFilePath)) {
						update_option('disciplinary_file', $filename);
					}
				}

				$data['file'] = isset($filename) ? $filename : '';

				$id = $this->hr_profile_model->add_disciplinary_history($data);
				if ($id) {
					$success = true;
					$message = _l('added_successfully', _l('disciplinary_action_history'));
					set_alert('success', $message);
				}
				redirect(admin_url('hr_profile/member/' . $data['staff_id'] . '/disciplinary_action_history'));
			} else {
				$id = $data['id'];
				unset($data['id']);
				...
				$success = $this->hr_profile_model->update_disciplinary_history($data, $id);
				if ($success) {
					$message = _l('updated_successfully', _l('disciplinary_action_history'));
					set_alert('success', $message);
				} else {
					$message = _l('hr_updated_failed', _l('disciplinary_action_history'));
					set_alert('warning', $message);
				}
				redirect(admin_url('hr_profile/member/' . $data['staff_id'] . '/disciplinary_action_history'));
			}
	...
	public function issued_assets_bnefits($id = '')
	{
		...
		if ($post) {
			$post['issued_assets_bnefits_name'] = isset($post['issued_assets_bnefits_name']) ? $post['issued_assets_bnefits_name'] : '';
			$post['returned'] = isset($post['returned']) ? $post['returned'] : 0;
		}
		if ($_FILES['file']['name'] != "") {
			$filetmpDir = module_dir_path('hr_profile/uploads/issued_assets_benefits');
			$tmpFilePath = $_FILES['file']['tmp_name'];
			$path_parts = pathinfo($_FILES['file']['name']);
			$extension  = $path_parts['extension'];
			$extension  = strtolower($extension);
			...
			if (move_uploaded_file($tmpFilePath, $newFilePath)) {
				update_option('issued_assets_benefits', $filename);
			}
		} else {
			if ($post['issued_assets_bnefits_id'] > 0 && $post['issued_assets_bnefits_id'] != '') {
				$id = $post['issued_assets_bnefits_id'];
				$issued_assets_benefits = $this->hr_profile_model->get_issued_assets_benefits($id);
				$filename = $issued_assets_benefits->file;
			}
		}
		$post['file'] = isset($filename) && $filename != '' ? $filename : '';
		if ($post['issued_assets_bnefits_id'] > 0 && $post['issued_assets_bnefits_id'] != '') {

			$id = $post['issued_assets_bnefits_id'];
			if (isset($post['issued_assets_bnefits_id'])) {
				unset($post['issued_assets_bnefits_id']);
			}
			$success = $this->hr_profile_model->update_issued_assets_benefits($post, $id);
			...
			if ($page != 'true') {
				redirect(admin_url('hr_profile/member/' . $post['staff_id'] . '/issued_assets_benefits'));
			} else {
				redirect(admin_url('hr_profile/issued_assets'));
			}
		} else {
			if (isset($post['issued_assets_bnefits_id'])) {
				unset($post['issued_assets_bnefits_id']);
			}
			$id = $this->hr_profile_model->add_issued_assets_benefits($post);
			if ($id) {
				$success = true;
				$message = _l('added_successfully', _l('issued_assets_benefits_add'));
				set_alert('success', $message);
			}
			if ($page != 'true') {
				redirect(admin_url('hr_profile/member/' . $post['staff_id'] . '/issued_assets_benefits'));
	...
	public function table_issued_assets_bnefits()
	{
		$this->app->get_table_data(module_views_path('hr_profile', 'hr_record/table_issued_assets_bnefits'));
	}

	public function get_issued_assets_benefits($id = '')
	{
		$filetmpDir = base_url() . 'modules/' .  HR_PROFILE_MODULE_NAME . '/uploads/issued_assets_benefits';
		$issued_assets_benefits = $this->hr_profile_model->get_issued_assets_benefits($id);
		$file_path = $filetmpDir . '/' . $issued_assets_benefits->file;
		echo json_encode([
			'issued_assets_benefits' => $issued_assets_benefits,
			'file_path' => $file_path
		]);
		die;
	}

	public function delete_issued_assets_bnefits($id = '')
	{
		if ($id != '' && $id > 0) {
			$filetmpDir_o = base_url() . 'modules/' .  HR_PROFILE_MODULE_NAME . '/uploads/issued_assets_benefits';
			$filetmpDir = module_dir_path('hr_profile/uploads/issued_assets_benefits');
			$issued_assets_benefits = $this->hr_profile_model->get_issued_assets_benefits($id);
			...
			$delete_issued_assets_benefits = $this->hr_profile_model->delete_issued_assets_benefits($id);
			if ($delete_issued_assets_benefits) {
				set_alert('success', _l('delete_issued_assets_benefits'));
			}
		}

		redirect(admin_url('hr_profile/member/' . $issued_assets_benefits->staff_id . '/issued_assets_benefits'));
		die;
	}

	public function issued_assets()
	{

		$this->load->model('staff_model');
		$data['title'] = _l('hr_profile_issued_assets');
		$data['main_page'] = true;
		$this->load->view('issued_assets/issued_assets_benifets', $data);
	}

	public function get_disciplinary_action_subject()
	{
		$data = $this->input->post();
		$id = $data['selected_action_category'];

		if (!empty($id)) {
			$disciplinary_subject = $this->hr_profile_model->get_disciplinary_category($id);
			echo json_encode($disciplinary_subject);
			exit;
		}
	}

	public function get_disciplinary_remark()
	{
		$data = $this->input->post();
		$id = $data['selected_action_remark'];

		if (!empty($id)) {
			$disciplinary_remark = $this->hr_profile_model->get_disciplinary_remark($id);
			echo json_encode($disciplinary_remark);
			exit;
		}
	}


	public function get_penalty_point()
	{
		$data = $this->input->post();
		$id = $data['selected'];

		if (!empty($id)) {
			$penalty_point = $this->hr_profile_model->get_penalty_template($id);
			echo json_encode($penalty_point->point);
			exit;
		}
	}

	public function print_discipline_data($id)
	{
		$discipline = $this->hr_profile_model->get_disciplinary_templetes($id);
		try {
			$pdf = discipline_note_pdf($discipline);
		} catch (Exception $e) {
			echo $e->getMessage();
			die;
		}
	...
	public function required_document_filed()
	{
		if ($this->input->post()) {
			$message = '';
			$data = $this->input->post();
			if (!$this->input->post('id')) {
				$id = $this->hr_profile_model->add_required_document_setting($data);
				if ($id) {
					$success = true;
					$message = _l('added_successfully');
					set_alert('success', $message);
				}
				redirect(admin_url('hr_profile/setting?group=required_document_201'));
			} else {
				$id = $data['id'];
				unset($data['id']);
				$success = $this->hr_profile_model->update_required_document_setting($data, $id);
				if ($success) {
	...
	public function delete_required_document($id)
	{
		if (!$id) {
			redirect(admin_url('hr_profile/setting?group=required_document_201'));
		}
		$response = $this->hr_profile_model->delete_required_document($id);
		if (is_array($response) && isset($response['referenced'])) {
	...
	public function edit_document_detail_staff($staff_id = '', $id = '')
	{
		$document_info = $this->hr_profile_model->get_document_info($staff_id, $id);
		$document_info->acquisition_date = _d($document_info->acquisition_date);
		$document_info->expiry_date = _d($document_info->expiry_date);

		echo json_encode($document_info);
		exit;
	}

	public function add_update_document_detail_staff()
	{
		$data = $this->input->post();
		$id = $data['document_id'];
		$staff_id = $data['add_document_staff_id'];
		$documentfileDir = HR_PROFILE_MODULE_UPLOAD_FOLDER . '/required_document_201/';

		// Retrieve current document info
		$getFiledata = $this->hr_profile_model->get_document_info($staff_id, $id);
		$oldfilename = $getFiledata ? $getFiledata->attach_file : '';
		...
		if (!empty($_FILES['attach_file']['tmp_name']) && $_FILES['attach_file']['tmp_name'] != '') {
			...
			$timestamp = time();
			$fileExtension = pathinfo($_FILES['attach_file']['name'], PATHINFO_EXTENSION);
			...
			$data['attach_file'] = $oldfilename;
		}

		$success = $this->hr_profile_model->add_update_document_info_cadidate($data);
		if ($success == true && !empty($filename)) {
			$removeoldfile = module_dir_path(HR_PROFILE_MODULE_UPLOAD_FOLDER . 'uploads/required_document_201/');
		...
		redirect(admin_url('hr_profile/member/' . $staff_id . '/required_document_201'));
	}
	...
	public function get_contract_data()
	{
		$data = $this->input->post();
		$id = $data['id'];
		$getdata = $this->hr_profile_model->get_contract_template($id);
