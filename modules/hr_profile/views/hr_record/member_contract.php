	...
	<div class="content">
		<div class="row">
			...
			<?php if ($hr_profile_member_add == false) { ?>
				<div class="col-md-3 member_contract">
					...
					<ul class="nav navbar-pills navbar-pills-flat nav-tabs nav-stacked">
						...
						?>
							<li<?php if ($group_item == $group || ($group_item == 'contract' && $group == 'contract_member')) {
									echo " class='active'";
								} ?>>
			...
			<?php } ?>
			<div class="<?php echo ($hr_profile_member_add == false) ? 'col-md-9 member_preview' : 'col-md-12 member_preview'; ?>">
				<div class="row">
					<div class="panel_s">
...
<script type="text/javascript">
	// for issued_assets_benefits
	var ContractsServerParams = {
		staff_id: "[name='staff_id']",
	};
	initDataTable('.table-table_issued_assets_bnefits', admin_url + "hr_profile/table_issued_assets_bnefits", [1], [1], ContractsServerParams);


	function edit_issued_assets_bnefits(invoker, id) {
		requestGetJSON("hr_profile/get_issued_assets_benefits/" + id).done(function(
			response
		) {
			console.log("response", response);
			$("#issued_assets_bnefits_name").val(response.issued_assets_benefits.issued_assets_bnefits_name);
			$('#issued_assets_bnefits_id').val(response.issued_assets_benefits.id);
			$("#image_download_anc").attr("href", response.file_path);
			$("#image_download_anc").html(response.issued_assets_benefits.file);
			if (response.issued_assets_benefits.returned == 1) {
				$('#returned').attr('checked', true);
			} else {
				$('#returned').attr('checked', false);
			}
			$("#issued_assets_bnefits_title").html('Edit Issued Assets Benefits')
		});
