...
<?php
$ci = get_instance();
$contract_templete_select = $ci->hr_profile_model->get_contract_template();
?>
<div class="row tab-left-0 ">
	<div class="col-md-12 member_preview">
			...
			<div class="horizontal-tabs">
				<ul class="nav nav-tabs tabs-in-body-no-margin contract-tab nav-tabs-horizontal mbot15" role="tablist">
					...
					<li role="presentation" class="tab-separator toggle_view">
						<a href="#" onclick="contract_full_view_member(); return false;" data-toggle="tooltip" data-title="<?php echo _l('toggle_full_view'); ?>">
...
<div class="tab-content">
	<div role="tabpanel" class="tab-pane<?php if (!$this->input->get('tab') || $this->input->get('tab') == 'contract_infor') {
											echo ' active';
										} ?>" id="contract_infor">
						...
						<tr class="project-overview">
							...
							?>
									<td class="text-right">
										...
										</a><?php echo new_html_entity_decode($s['firstname'] . ' ' . $s['lastname']); ?>
									</td>
								<?php } ?>
							<?php } ?>
						</tr>
	...
	<div role="tabpanel" class="tab-pane " id="contract">
		...
		<div class="row">
			<div class="col-md-6">
				<?php echo render_select('contract_templete_select',$contract_templete_select, ['id', 'name'], 'contract_templete_select', ''); ?>
			</div>

			<div class="col-md-6 text-right">
				<div class="btn-group">
					<a href="#" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
						<i class="fa fa-file-pdf-o"></i> <?php if (is_mobile()) {
																echo ' PDF';
															} ?> <span class="caret"></span>
					</a>
					<ul class="dropdown-menu dropdown-menu-right">
						<li class="d-none d-sm-block"><a href="<?php echo admin_url('hr_profile/contract_pdf/' . $contracts->id_contract . '?output_type=I'); ?>"><?php echo _l('view_pdf'); ?></a></li>
						<li class="d-none d-sm-block"><a href="<?php echo admin_url('hr_profile/contract_pdf/' . $contracts->id_contract . '?output_type=I'); ?>" target="_blank"><?php echo _l('view_pdf_in_new_window'); ?></a></li>
						<li><a href="<?php echo admin_url('hr_profile/contract_pdf/' . $contracts->id_contract); ?>"><?php echo _l('download'); ?></a></li>
						<li><a href="<?php echo admin_url('hr_profile/contract_pdf/' . $contracts->id_contract . '?print=true'); ?>" target="_blank"><?php echo _l('print'); ?></a></li>
					</ul>
				</div>

				<a href="#" class="btn btn-default d-none" data-target="#contract_send_to_client_modal" data-toggle="modal">
					<span class="btn-with-tooltip" data-toggle="tooltip" data-title="<?php echo _l('contract_send_to_email'); ?>" data-placement="bottom">
						<i class="fa fa-envelope"></i>
					</span>
				</a>

				<a href="<?php echo admin_url('hr_profile/contract_sign/' . $contracts->id_contract); ?>" class="btn btn-default">
					<span class="btn-with-tooltip" data-toggle="tooltip" data-title="<?php echo _l('View'); ?>" data-placement="bottom">
		...
		<div class="row">
			...
			<div class="col-md-6">
				<div id="tc-content" class="tc-content editable mce-content-body" style="border: 1px solid rgb(210, 210, 210); min-height: 70px; border-radius: 4px; position: relative;" id="mce_0" contenteditable="true" spellcheck="false">
					<p><span class="text-danger text-uppercase mtop15 editor-add-content-notice">&nbsp;<?php echo $contracts->content; ?><br></span></p>
				</div>
			</div>
...
<script>
	function contract_full_view_member() {
		$('.member_contract').toggleClass('hide');
		if ($('.member_preview').hasClass('col-md-9')) {
			$('.member_preview').removeClass('col-md-9').addClass('col-md-12');
		} else {
			$('.member_preview').removeClass('col-md-12').addClass('col-md-9');
		}
		$(window).trigger('resize');
	}

	$(document).ready(function() {
		$("select[name='contract_templete_select']").on("change", function() {
			var id = $(this).val();
			var contract_id = '<?php echo $contracts->id_contract;?>';
			var content = tinyMCE.get("tc-content").getContent();
			$.ajax({
				type: "post",
				url: admin_url + "hr_profile/get_contract_data",
				data: { id: id, content: content, contract_id: contract_id },
				success: function(response) {
					var data = JSON.parse(response);
					tinyMCE.get("tc-content").setContent(data.content);
