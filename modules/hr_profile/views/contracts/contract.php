									...
									<div class="col-md-6">
										...
										<select name="staff" class="selectpicker" id="staff" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>" data-live-search="true">
											...
											<?php foreach ($staff as $s) { ?>
												<option value="<?php echo new_html_entity_decode($s['staffid']); ?>" <?php if (isset($contracts) && $contracts->staff == $s['staffid'] || isset($staff_id) && $staff_id == $s['staffid']) {
																															echo 'selected';
																														} ?>> <?php echo new_html_entity_decode($s['firstname'] . ' ' . $s['lastname']); ?></option>
											<?php } ?>
										</select>
									</div>
								</div>
										...
										<div class="form-group">
											...
											<select name="name_contract" class="selectpicker" id="name_contract" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
												...
												<?php foreach ($contract_type as $c) { ?>
													<option value="<?php echo new_html_entity_decode($c['id_contracttype']); ?>" <?php if (isset($contracts) && $contracts->name_contract == $c['id_contracttype']) {
																																		echo 'selected';
																																	} ?>><?php echo new_html_entity_decode($c['name_contracttype']); ?> </option>
												<?php } ?>
											</select>
										</div>
									</div>
									...
									<div class="col-md-6">
										<div class="form-group">
											...
											<select name="contract_status" class="selectpicker" id="contract_status" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
												<option value="draft" <?php if (isset($contracts) && $contracts->contract_status == 'draft') {
																			echo 'selected';
																		} ?>><?php echo _l('hr_hr_draft') ?></option>
												<option value="valid" <?php if (isset($contracts) && $contracts->contract_status == 'valid') {
																			echo 'selected';
																		} ?>><?php echo _l('hr_hr_valid') ?></option>
												<option value="invalid" <?php if (isset($contracts) && $contracts->contract_status == 'invalid') {
																			echo 'selected';
																		} ?>><?php echo _l('hr_hr_expired') ?></option>
												<option value="finish" <?php if (isset($contracts) && $contracts->contract_status == 'finish') {
																			echo 'selected';
																		} ?>><?php echo _l('hr_hr_finish') ?></option>
											</select>
										</div>
									</div>
									...
									<div class="col-md-6">
										<div class="form-group">
											...
											<select name="hourly_or_month" class="selectpicker" id="hourly_or_month" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
												<option value="month" <?php if (isset($contracts) && $contracts->hourly_or_month == 'month') {
																			echo 'selected';
																		} ?>><?php echo _l('hr_month') ?></option>
												<option value="hourly_rate" <?php if (isset($contracts) && $contracts->hourly_or_month == 'hourly_rate') {
																				echo 'selected';
																			} ?>><?php echo _l('hourly_rate') ?></option>
									...
									<div class="row">
										...
										<div class="col-md-6">
											...
											<select name="staff_delegate" class="selectpicker" data-live-search="true" id="staff_delegate" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
												<option value=""></option>
												<?php foreach ($staff as $s) { ?>
													<option value="<?php echo new_html_entity_decode($s['staffid']); ?>" <?php if (isset($contracts) && $contracts->staff_delegate == $s['staffid']) {
																																echo 'selected';
																															} ?>> <?php echo new_html_entity_decode($s['firstname'] . ' ' . $s['lastname']); ?></option>
												<?php } ?>
											</select>
