...
foreach ($rResult as $aRow) {
	...
	if (has_permission('hrm_contract', '', 'view') || has_permission('hrm_contract', '', 'view_own') || is_admin()) {
		$member = $this->ci->input->post('memberid');
		
		$subjectOutput = '<a href="' . admin_url('hr_profile/member_contract_view/'.$member .'/contract_member/'. $aRow['id'] ).'" onclick="init_hrm_contract(' . $aRow['id'] . ');return false;">' . $aRow['contract_code'] . '</a>';
	}else{
