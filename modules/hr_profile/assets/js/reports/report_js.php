...
<script>
	...
	var fnServerParams = {
		...
		"staff_filter_new": "[name='staff_filter[]']",
		"staff_filter_contract": "[name='staff_filter_new[]']",
		"contract_code": "[name='contract_code[]']",
    ...
    'use strict';
	 ...
	 $('.sorting_table_filter').addClass('hide');
	 $('.sorting_table_contract_code').addClass('hide');
	 $('select[name="position[]"]').closest('.col-md-4').removeClass('hide');
	 $('select[name="staff[]"]').closest('.col-md-4').removeClass('hide');
	 $('select[name="staff_filter[]"]').closest('.col-md-4').removeClass('hide');
	 $('select[name="staff_filter_new[]"]').closest('.col-md-4').removeClass('hide');
	 $('select[name="contract_code[]"]').closest('.col-md-4').removeClass('hide');
	 ...
	 if(type == 'seniority'){
		...
		$('#list_of_discipline_report').addClass('hide');
		$('#list_of_employee_contract_report').addClass('hide');
	 ...
	 } else if(type == 'list_of_discipline_report'){
		
		$('.sorting_table_filter').removeClass('hide');
		$('#hr_is_working').addClass('hide');
		$('#container_senior_staff').addClass('hide');
		$('.leave-statistics-gen').addClass('hide');
		$('.working-hours-gen').addClass('hide');
		...
		$('#report_the_employee_quitting').addClass('hide');
		$('#list_of_employees_with_salary_change').addClass('hide');
		$('#list_of_discipline_report').removeClass('hide');
		$('#qualifications_department').addClass('hide');
		$('#additional_timesheets').addClass('hide');
		$('#list_of_employee_contract_report').addClass('hide');
		...
		$('#report-time').removeClass('hide');
		$('#year_requisition').addClass('hide');
	 } else if(type == 'list_of_employee_contract_report'){
		
		$('.sorting_table_contract_code').removeClass('hide');
		$('#hr_is_working').addClass('hide');
		$('#container_senior_staff').addClass('hide');
		$('.leave-statistics-gen').addClass('hide');
		$('.working-hours-gen').addClass('hide');
		...
		$('#list_of_employees_with_salary_change').addClass('hide');
		$('#list_of_discipline_report').addClass('hide');
		$('#list_of_employee_contract_report').removeClass('hide');
		$('#qualifications_department').addClass('hide');
		$('#additional_timesheets').addClass('hide');
    ...
    'use strict';
		...
		if(!$('#list_of_discipline_report').hasClass('hide')){
			list_of_discipline_report();
		}
		if(!$('#list_of_employee_contract_report').hasClass('hide')){
			list_of_employee_contract_report();
		}
...
$(function() {
	...
	$('select[name="position[]"], select[name="department[]"], select[name="staff[]"], select[name="staff_filter[]"], select[name="staff_filter_new[]"], select[name="contract_code[]"]').on('change', function() {
		gen_reports();
	});
...
function list_of_discipline_report(){
    'use strict';

	$('.title_table').text('<?php echo _l('table-table_disciplinary_history'); ?>');
	if ($.fn.DataTable.isDataTable('.table-table_disciplinary_history')) {
		$('.table-table_disciplinary_history').DataTable().destroy();
	} 
	initDataTable('.table-table_disciplinary_history', admin_url + 'hr_profile/list_of_discipline_report', [0], [0], fnServerParams, [0, 'desc']);

	//hide first column
    // var hidden_columns = [0];
    //     $('.table-table_disciplinary_history').DataTable().columns(hidden_columns).visible(false, false);
}

function list_of_employee_contract_report(){
    'use strict';

	$('.title_table').text('<?php echo _l('table-table_employee_contract'); ?>');
	if ($.fn.DataTable.isDataTable('.table-table_employee_contract')) {
		$('.table-table_employee_contract').DataTable().destroy();
	} 
	initDataTable('.table-table_employee_contract', admin_url + 'hr_profile/list_of_employee_contract_report', [0], [0], fnServerParams, [0, 'desc']);

	//hide first column
    // var hidden_columns = [0];
    //     $('.table-table_employee_contract').DataTable().columns(hidden_columns).visible(false, false);
}
