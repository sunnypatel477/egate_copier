...
/*
Module Name: HRMS
Description: The primary function of HRMS is to provide a central database containing records for all employees past and presen
Version: 1.0.9
...
{   
	...
	// if(has_permission('hrm_contract','','view') || has_permission('hrm_contract','','view_own')){
		$CI->app_menu->add_sidebar_children_item('hr_profile', [
			'slug'     => 'hr_profile_issued_assets',
			'name'     => _l('hr_profile_issued_assets'),
			'icon'     => 'fa-solid fa-file-contract',
			'href'     => admin_url('hr_profile/issued_assets'),
			'position' => 6,
		]);
	// }
...
function hr_profile_load_js(){    
	...
	if(!(strpos($viewuri,'admin/hr_profile/setting?group=disciplinary_templates') === false)){
		echo '<script src="'.module_dir_url(HR_PROFILE_MODULE_NAME, 'assets/js/setting/disciplinary_templates.js').'?v=' . VERSION_HR_PROFILE.'"></script>';
	}

	if(!(strpos($viewuri,'admin/hr_profile/setting?group=disciplinary_action_templates') === false)){
		echo '<script src="'.module_dir_url(HR_PROFILE_MODULE_NAME, 'assets/js/setting/disciplinary_action_templates.js').'?v=' . VERSION_HR_PROFILE.'"></script>';
	}

	if(!(strpos($viewuri,'admin/hr_profile/setting?group=required_document_201') === false)){
		echo '<script src="'.module_dir_url(HR_PROFILE_MODULE_NAME, 'assets/js/setting/required_document_201.js').'?v=' . VERSION_HR_PROFILE.'"></script>';
	}
	...
	if(!(strpos($viewuri,'admin/hr_profile/member') === false)){
		...
		if(!(strpos($viewuri,'disciplinary_action_history') === false)){
			echo '<script src="'.module_dir_url(HR_PROFILE_MODULE_NAME, 'assets/js/hr_record/includes/disciplinary_action_history.js').'?v=' . VERSION_HR_PROFILE.'"></script>';
		}
