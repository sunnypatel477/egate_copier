...
class App_items_table extends App_items_table_template
{
    ...
    public function items()
    {
        ...
        foreach ($this->items as $item) {
            ...
            if($item['rel_type'] == 'purchase_order') {
                $itemHTML .= '<td style="color:#0000FF" align="right" width="' . $regularItemWidth . '%">' . e(floatVal($item['qty']) - get_total_sum_of_delivery_notes($item['rel_id'], $item['item_order'])) . '</td>';
            }
    ...
    public function html_headings()
    {
        ...
        if($table_type == 'purchase_order') {
            $html .= '<th align="right">' . _l('bal') . '</th>';
        }
