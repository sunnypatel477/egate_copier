...
if (!function_exists('format_proposal_info')) {
    ...
    function format_proposal_info($proposal, $for = '')
    {
        ...
        $company_name = get_company_name_proposal($proposal->rel_id);

        // $proposalTo = '<b>' . $proposal->proposal_to . '</b>';
        if($proposal->rel_type == 'lead') {
            $lead_name = get_lead_name_proposal($proposal->rel_id);
            $proposalTo = '<b>' . $proposal->proposal_to . '</b><br><b>Attn: '.$lead_name->name.'</b>';
        } elseif($proposal->rel_type == 'customer') {
            $company_name = get_company_name_proposal($proposal->rel_id);
            $proposalTo = '<b>' . $company_name->company . '</b><br><b>Attn: '.$proposal->proposal_to.'</b>';
        }
