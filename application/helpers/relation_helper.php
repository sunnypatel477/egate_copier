...
function get_relation_data($type, $rel_id = '', $extra = [])
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        if ($rel_id != '') {
            ...
            $CI->load->model('Client_lead_payments_model');
            $search = $CI->Client_lead_payments_model->_search_client_leads($q, 0, [
                'junk' => 0,
                ]);
            $data = $search['result'];
        }
    } elseif ($type == 'lead_customer' || $type == 'lead_customers') {
            $CI->load->model('Client_lead_payments_model');
            $search = $CI->Client_lead_payments_model->_search_customer_leads($q, 0, [
                'junk' => 0,
                ]);
            $data = $search['result'];
    } elseif ($type == 'proposal') {
