        ...
        <div class="panel-body">
            <?php foreach ($invoices_to_merge as $_inv) { ?>
                <div class="tw-flex tw-justify-between tw-items-center tw-mb-2 last:tw-mb-0">
                    <div>
                        <a href="<?php echo admin_url('invoices/list_invoices/' . $_inv->id); ?>" target="_blank" class="tw-font-medium"><?php echo format_invoice_number($_inv->id); ?></a> -
        ...
        <div class="panel-body">
                    ...
                    <div class="pull-right">
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li class="hidden-xs"><a href="<?php echo admin_url('invoices/pdf/' . $invoice->id . '?output_type=I'); ?>"><?php echo _l('view_pdf'); ?></a>
                                </li>
                                ...
                                <li><a href="<?php echo admin_url('invoices/pdf/' . $invoice->id); ?>"><?php echo _l('download'); ?></a>
                                </li>
                                <li>
                        ...
                        <?php if (!empty($invoice->clientid)) { ?>
                            <span<?php if ($invoice->status == Invoices_model::STATUS_CANCELLED) { ?> data-toggle="tooltip" data-title="<?php echo _l('invoice_cancelled_email_disabled'); ?>" <?php } ?>>
                                <a href="#" class="invoice-send-to-client btn-with-tooltip btn btn-default<?php if ($invoice->status == Invoices_model::STATUS_CANCELLED) {
                                                                                                                echo ' disabled';
                                                                                                            } ?>" data-toggle="tooltip" title="<?php echo ($_tooltip); ?>" data-placement="bottom"><span data-toggle="tooltip" data-title="<?php echo ($_tooltip_already_send); ?>"><i class="fa-regular fa-envelope"></i></span></a>
                                </span>
                            <?php } ?>
                            <!-- Single button -->
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    <li><a href="<?php echo site_url('invoice/' . $invoice->id . '/' . $invoice->hash) ?>" target="_blank"><?php echo _l('view_invoice_as_customer_tooltip'); ?></a>
                                    </li>
                                    <li>
                                    ...
                                    <?php } ?>
                                    <?php if (
                                        !in_array($invoice->status, [Invoices_model::STATUS_PAID, Invoices_model::STATUS_CANCELLED, Invoices_model::STATUS_DRAFT])
            ...
            <div class="tab-content">
                ...
                <?php if (count($invoice->payments) > 0) { ?>
                    ...
                    <?php foreach ($invoice->payments as $attachment) { ?>

                        <div role="tabpanel" class="tab-pane" id="tab_files">
                            <?php if ($attachment['invoice_hard_copy'] != '') { ?>
                                <div class="col-md-12">
                                    <?php
                                    $invoicetmpDir = module_dir_url('client_lead/asset/invoice_hard_copy');
                                    $invoice_hard_copy = $invoicetmpDir . $attachment['invoice_hard_copy'];
                                    ?>
                                    <label for="invoice_hard_copy"><?php echo _l('invoice_hard_copy'); ?> : </label>
                                    <a href="<?php echo $invoice_hard_copy; ?>" target="_blank"><?php echo $attachment['invoice_hard_copy']; ?></a>
                                </div>
                            <?php } ?>
                            <?php if ($attachment['payment_prof'] != '') { ?>
                                <div class="col-md-12">
                                    <?php
                                    $paymentproftmpDir = module_dir_url('client_lead/asset/payment_prof');
                                    $payment_prof = $paymentproftmpDir . $attachment['payment_prof'];
                                    ?>
                                    <label for="payment_prof"><?php echo _l('payment_prof'); ?> : </label>
                                    <a href="<?php echo $payment_prof; ?>" target="_blank"><?php echo $attachment['payment_prof']; ?></a>
                                </div>
                            <?php } ?>
                            <?php if ($attachment['officialreceipt'] != '') { ?>
                                <div class="col-md-12">
                                    <?php
                                    $officialreceipttmpDir = module_dir_url('client_lead/asset/officialreceipt');
                                    $official_receipt = $officialreceipttmpDir . $attachment['officialreceipt'];
                                    ?>
                                    <label for="official_receipt"><?php echo _l('official_receipt'); ?> : </label>
                                    <a href="<?php echo $official_receipt; ?>" target="_blank"><?php echo $attachment['officialreceipt']; ?></a>
                                </div>
                            <?php } ?>
                            <?php if ($attachment['taxdeduction2307'] != '') { ?>
                                <div class="col-md-12">
                                    <?php
                                    $taxdeduction2307tmpDir = module_dir_url('client_lead/asset/taxdeduction2307');
                                    $tax_deduction_2307 = $taxdeduction2307tmpDir . $attachment['taxdeduction2307'];
                                    ?>
                                    <label for="tax_deduction_2307"><?php echo _l('tax_deduction_2307'); ?></label>
                                    <a href="<?php echo $tax_deduction_2307; ?>" target="_blank"><?php echo $attachment['taxdeduction2307']; ?></a>
                                </div>
                            <?php } ?>
                        </div>
                    <?php } ?>
                ...
                <?php if (count($invoice->payments) > 0) { ?>
                    <div class="tab-pane" role="tabpanel" id="invoice_payments_received">
                        <?php
                        include_once(module_dir_path(CLIENT_LEAD_MODULE, 'views/tables/invoice_payments_table.php'));
                        ?>
                    </div>
                <?php } ?>
                <?php if (count($applied_credits) > 0) { ?>
                ...
                <div role="tabpanel" class="tab-pane" id="tab_reminders">
                    <a href="#" class="btn btn-default" data-toggle="modal" data-target=".reminder-modal-invoice-<?php echo ($invoice->id); ?>"><i class="fa-regular fa-bell"></i>
                            ...
                            <ul class="list-group">
                                <?php foreach ($invoice_recurring_invoices as $recurring) { ?>
                                    <li class="list-group-item">
                                        <a href="<?php echo admin_url('invoices/list_invoices/' . $recurring->id); ?>" class="tw-font-semibold" onclick="init_invoice(<?php echo ($recurring->id); ?>); return false;" target="_blank"><?php echo (format_invoice_number($recurring->id)); ?>
