...
return App_table::find('clients')
    ->outputUsing(function ($params) {
        ...
        foreach ($rResult as $aRow) {
            ...
            $pendingTaskCount = pending_task_count($aRow['userid']);

            // Construct the row array
            $row[] = ($aRow['lead'] ? '<span title="'._l('total_lead_title').'" class="lead-count">' . $aRow['lead'] . '</span> ' : '') . ($pendingTaskCount > 0 ? '<span title="'._l('pending_task_title').'"class="task-count"> ' . $pendingTaskCount . '</span>' : '');
