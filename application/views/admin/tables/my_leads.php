...
return App_table::find('leads')
    ->outputUsing(function ($params) use ($statuses) {
        ...
        $aColumns = array_merge($aColumns, [
            ...
            'lead_value',
            '(SELECT GROUP_CONCAT(name SEPARATOR ",") FROM ' . db_prefix() . 'taggables JOIN ' . db_prefix() . 'tags ON ' . db_prefix() . 'taggables.tag_id = ' . db_prefix() . 'tags.id WHERE rel_id = ' . db_prefix() . 'leads.id and rel_type="lead" ORDER by id ASC LIMIT 1) as tags',
        ...
        array_push($where, 'AND existing_client_id IS NULL');
